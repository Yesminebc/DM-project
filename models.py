...
class HeuristicModel(BaseModel):
    ...
    def visualize_clusters(self, X, Y):
        ...
        # Ensure both X and Y have the same number of dimensions
        if X.ndim == 1:
            X = X.reshape(-1, 1)
        if Y.ndim == 1:
            Y = Y.reshape(-1, 1)
        ...
        scatter = plt.scatter(reduced_data[:, 0], reduced_data[:, 1], c=cluster_labels, cmap='viridis', edgecolor='k', alpha=0.7)

        # Plot cluster centers
        if self.cluster_centers_ is not None:
            reduced_centers = pca.transform(self.cluster_centers_)
